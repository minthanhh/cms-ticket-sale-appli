...
import { onOpenModalChangeDateTicketUse, onOpenModalFillter } from '@/store/slices/modalAddTickets'
...
const FamillyPackage = () => {
    ...
    const [toggleTooltip, setTooltip] = useState(Array(dataTable.length).fill(false))

    // const [filterData, setFilterData] = useState<ITicketPackage[]>([]);

    const { filteredData, globalSearch, setSearchText, searchText, setFilteredData } = useGlobalFilter(dataTable)
        ...
        },
        {
            ...
            render(value, record, index) {
                ...
                return record.usageStatus === 'notUsedYet' ? (
                    <>
                        <BsThreeDotsVertical size={24} className="cursor-pointer" onClick={() => handleClick(index)} />
                        {toggleTooltip[index] ? (
                            <div className="bg-[#FFD2A8] shadow-md absolute top-0 right-full overflow-hidden flex items-center flex-col justify-center w-max rounded-lg">
                                <button className="px-3 py-2 w-full hover:bg-[#caa786] default-animate" onClick={() => dispatch(startUsingTicket(record.id as string))}>
                                    Sử dụng vé
                                </button>
                                <button
                                    onClick={() => {
    ...
    useEffect(() => {
        const familyPackage = listTicketPackage.filter((i) => i.ticketPackageName === 'Gói gia đình')
        ...
        <>
            <div className="flex items-center justify-between mb-[31px]">
                ...
                <div className="flex flex-row items-center gap-[10px]">
                    <Button title="Lọc vé" outline icon={FiFilter} onClick={() => dispatch(onOpenModalFillter())} />
                    <Button title="Xuất file (.csv)" outline onClick={() => exportToCSV(dataTable)} />
                </div>
            </div>
            ...
            <Table
                ...
                className="w-full"
                columns={columns}
                dataSource={filteredData && filteredData.length ? filteredData : dataTable}
                pagination={{
